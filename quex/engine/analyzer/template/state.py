...
class TemplateState(AnalyzerState):
    ...
    def __init__(self, StateA, StateB):
        ...
        self.__state_index_to_state_key_db = dict((state_index, i) for i, state_index in self.__state_index_list)

        # Combined DropOut and Entry schemes are generated by the same function
        self.__update_entry(self.entry, self.__state_index_to_state_key_db, StateA)
        self.__update_entry(self.entry, self.__state_index_to_state_key_db, StateB)
    ...
    def __update_entry(self, TheState):
        if isinstance(TheState, AnalyzerState):
            self.entry.action_db_update(                                    \
                 (AdvancedFromStateIndex(StateA.index, x.from_state_index), \
                  x.action_list)                                            \
                 for x in TheState.entry.action_db_itervalues())
        elif isinstance(TheState, TemplateState):
            self.entry.action_db_update(TheState.entry.action_db)
            self.__setup_SetStateKey(self.entry)
        else:
            assert False

    def __setup_SetStateKey(self, StateIndex, TheCommandList):
        """If there is an action 'SetStateKey' in the action list, then 
           reset it so that the correct state key for the given state 
           is set. If there is none then generate a new SetStateKey action.
        """
        for action in TheCommandList.misc:
            if isinstance(action, SetStateKey):
                action.value = self.__state_index_to_state_key_db[StateIndex]
                break
        else:
            action = entry_action.SetStateKey(self.__state_index_to_state_key_db[StateIndex])
            action_list.misc.add(action)
