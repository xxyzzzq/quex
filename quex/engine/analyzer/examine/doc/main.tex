...
non-constant, the combination of entry operations $op^v_E(i,k) = \{ A(v) :=
R_E(i,k) \}$ and the output recipe $R^v(i)=\{ v:=A(v) \}$ remains a correct
...
\ref{eq:homogeneity-condition}.  For run-time interference, $op^v(i)$ has been
overtaken to the output recipe $R^v(i)$ if it was constant in itself.  With
equation \ref{eq:homogeneity-condition} and determined entries $R_E(i,k)$ it is
...
must be implemented according to $A(v) := R_E^v(i,k)$.
