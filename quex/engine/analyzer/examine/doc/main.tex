...
applied to the variables $V_c$ upon entry into the state machine, independent
from where the state is entered. Let $op_E(i,k)$ name operations applied upon
entry, depending on the state from which they are entered.
...
that the line number variable should be increased upon each newline character
...
operation $op(i)$ takes the variable settings $V(k)$ of the previous state $k$
...
The goal of all analysis is to exploit deterministic characteristics to omit
redundant some operations along transitions.  Only when exiting the state
...
determined, then the state's recipe by equation \eqref{eq:entry-recipe-concatenated} 
...
$\varepsilon$ denote the not existing element. That is, if $\varepsilon$ is added to a
...
\begin{definition} Spring
    ...
    A spring is a state $i$ where all components $R^v(i)$ $\forall\,v\in\,V(i)$
    are determined.
...
\end{definition}
%%
Both types of states, linear and mouth states, may be springs.  Initial springs
can be the starting point for the development of recipes, because they can derive
a recipe independently of the predecessor's recipe. 

\subsection{Examples}
                 
Figure \ref{fig:two-state-machines}.a displayed an state machine with a single
variable, i.e. $V_c=\{'x'\}$. The operations $op(i)$ upon transition are
$x:=x+1$ for all states. That is $x$ is incremented by 1 at each step. Let
$A_a(x)$ hold the value of $x$ upon entry into the state machine.  This
notation combines the recipe's procedure with its snapshot map.  In the above
example, the recipe in state $a$ solely restores what has been stored.
%%
\begin{equation} 
    R(a) = \{ x := A_a(x) \} 
\end{equation}
...
'tree walk'.  The termination criteria for the 'accumulation walk' walk is
defined as follows.
...
omitted from consideration of indeterminacy.
...
to transition paths, this proves that loops must be involved. Loop, or circular
...
interference remains correct $\blacksquare$.  

The output recipes can be used for propagation of recipes until other dead-lock
states receive determined entries.  The horizon moves, the set of dead-lock
states shrinks and a new cycle begins. This continues until no dead-lock state
is left.  Eventually, all entry recipes are be determined. Then, it is possible
that those entries which are flagged with $F_E^v(i,k)=true$ implement the entry
operation $op_E^v(i)=R_E^v(i,k)$. But, wait! There might be some fine-tuning
coming up.
...
\ref{eq:homogeneity-condition}.  For run-time interference, before, 
$op^v(i)$ has been overtaken to the output recipe $R^v(i)$ if it was constant
in itself.  Now, that all entry recipes are determined, homogeneity can be
achieved if the concatenation of $op^v(i)\circ R(k)$ for all $k\,\in\,Pred(i)$
are of the same constant.  Consider the example of 
...
be determined. Thus the fine-tuning has reached its optimum.  Once, fine tuning
is done the remaining entry operations $op_E^v(i)$ for which $F_E^v(i,k)=true$
must be implemented according to $R_E^v(i,k)$.
