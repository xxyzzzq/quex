...
for an arbitrary $R_E^v(i,k_0)\neq\eta$. If $v$ is incoherent, an entry
operation $op^v_E(i,k)$ must be implemented in the resulting state machine that
stores the computed value of $v$ in $A(v)$.  For the snapshot sets of the
result recipe $R(i)$ it holds

\begin{equation} \label{eq:interference-snapshot-sets}
    S^v(R(i)) = \bigcap_{k\in Pred(i)} S^v(R_E(i,k))
\end{equation}
...
The implementation of entry operations $op^v_E(i,k)$ can be post-poned to the
point in time, when all recipes are determined. It is enough to maintain a
'coherence database', that tells for a given state whether a variable develops
coherently or not. Eventually, when all recipe analysis is done, the entry
...
\begin{equation} \label{eq:entry-operation-implementation}
    op_E^v(i,k) \,=\, \{ A(v) \,=\, R^v_E(i,k) \} \,\mbox{if $H^v(i) \neq true$}.
\end{equation}
...
history-independent, then any dependency on $A$ is removed and $S^v(R(v))$
becomes the not existing element $\varepsilon$. In the incoherent case, when a
snapshot of a variable $v$ is taken in state $i$, then the set of snapshot
states $S(R(i))$ must contain $i$. The existence of the entry operations
$op_E(i,k)$ is the reason why the result is expressed as a multi-entry state
machine is required. 
...
dead-locks.  In particular, if the initial state is undetermined, it becomes
...
\begin{definition} Daring Estimate

    The daring estimate proposes an output recipe $R(i)$ for a horizon state
    $h$ with $P_d(h)$ as the set of its determined predecessor states. The
    recipes components for each $v$ are defined by
...
for an arbitrary $k\in P_u(h)$ with $R_E^v(i,k_0)\neq\eta$.  The snapshot set
for each variable $v$ is the union of snapshot sets of all determined states.

\begin{equation} \label{eq:interference-snapshot-sets}
    S^v(R(i)) = \bigcap_{k\in Pred(i)} S^v(R_E(i,k))\,\mbox{with $k$ being determined.}
\end{equation}

\end{definition}

Without knowing the other entry recipes, it can be said that the recipe
resulting daring interference is true for transition sequences that

\begin{itemize}
    \item Enter the mouth state through a determined entry, and
    \item do not enter is through an, yet, undetermined entry.
\end{itemize}
