...
range_skipper_template = """
{
    ...
    QUEX_TYPE_CHARACTER*        text_end = QuexBuffer_text_end(&engine->buffer);
$$LC_COUNT_COLUMN_N_POINTER_DEFINITION$$

$$ENTRY$$
    QUEX_BUFFER_ASSERT_CONSISTENCY(&engine->buffer);
    __quex_assert(QuexBuffer_content_size(&engine->buffer) >= Skipper$$SKIPPER_INDEX$$L );
...
$$LC_COUNT_IN_LOOP$$
    ...
    *text_end = QUEX_SETTING_BUFFER_LIMIT_CODE; /* Reset BLC. */
     ...
     *
     */
    if( QuexBuffer_distance_input_to_text_end(&engine->buffer) == 0 ) {
        /* (1) */
        $$GOTO_DROP_OUT$$            
    } 
    else if( Skipper$$SKIPPER_INDEX$$L && QuexBuffer_distance_input_to_text_end(&engine->buffer) < Skipper$$SKIPPER_INDEX$$L ) {
        /* (2.1) */
        $$GOTO_DROP_OUT$$            
    }
...
$$LC_COUNT_BEFORE_RELOAD$$
    if(    QuexAnalyzerEngine_buffer_reload_forward(&engine->buffer, &last_acceptance_input_position,
        ...
        text_end = QuexBuffer_text_end(&engine->buffer);
...
$$LC_COUNT_COLUMN_N_POINTER_DEFINITION$$
    ...
    QUEX_BUFFER_ASSERT_CONSISTENCY(&engine->buffer);
    __quex_assert(QuexBuffer_content_size(&engine->buffer) >= 1);
    if( QuexBuffer_distance_input_to_text_end(&engine->buffer) == 0 ) 
        $$GOTO_DROP_OUT$$
...
$$LC_COUNT_BEFORE_RELOAD$$
        ...
        if( QuexAnalyzerEngine_buffer_reload_forward(&engine->buffer, &last_acceptance_input_position,
            ...
            QUEX_BUFFER_ASSERT_CONSISTENCY(&engine->buffer);
            QuexBuffer_input_p_increment(&engine->buffer);
$$LC_COUNT_AFTER_RELOAD$$
            $$GOTO_LOOP_START$$
        } else {
            $$GOTO_TERMINAL_EOF$$
...
#           if defined(QUEX_OPTION_COLUMN_NUMBER_COUNTING)
            column_count_p_$$SKIPPER_INDEX$$ = QuexBuffer_tell_memory_adr(&engine->buffer);
...
def __range_skipper_lc_counting_replacements(code_str, EndSequence):
    ...
    variable_definition = \
      ...
      "    QUEX_TYPE_CHARACTER_POSITION column_count_p_$$SKIPPER_INDEX$$ = QuexBuffer_tell_memory_adr(&engine->buffer);\n"+\
      "#   endif\n" + \
      "#   endif\n"
    in_loop       = ""
    end_procedure = ""
    ...
    if index != -1:
        ...
        end_procedure = "#       ifdef QUEX_OPTION_COLUMN_NUMBER_COUNTING\n" + \
                        "        self.counter.base._column_number_at_end +=   QuexBuffer_tell_memory_adr(&engine->buffer)\n" + \
