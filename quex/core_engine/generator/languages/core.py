...
db["C++"] = {
    ...
    "$if EOF":              "if( QUEX_NAME(Buffer_is_end_of_file)(&engine->buffer) ) {\n",
    "$if BOF":              "if( QUEX_NAME(Buffer_is_begin_of_file)(&engine->buffer) ) {\n",
    ...
    "$input":               "input",
    "$mark-lexeme-start":   "QUEX_NAME(Buffer_mark_lexeme_start)(&engine->buffer);",
    "$input/increment":     "QUEX_NAME(Buffer_input_p_increment)(&engine->buffer);",
    "$input/add":           lambda Offset:      "QUEX_NAME(Buffer_input_p_add_offset)(&engine->buffer, %i);" % Offset,
    "$input/decrement":     "QUEX_NAME(Buffer_input_p_decrement)(&engine->buffer);",
    "$input/get":           "input = QUEX_NAME(Buffer_input_get)(&engine->buffer);",
    "$input/get-offset":    lambda Offset:      "input = QUEX_NAME(Buffer_input_get_offset)(&engine->buffer, %i);" % Offset,
    "$input/tell_position": lambda PositionStr: "%s = QUEX_NAME(Buffer_tell_memory_adr)(&engine->buffer);\n" % PositionStr,
    "$input/seek_position": lambda PositionStr: "QUEX_NAME(Buffer_seek_memory_adr)(&engine->buffer, %s);\n" % PositionStr,
    "$return":              "return;",
