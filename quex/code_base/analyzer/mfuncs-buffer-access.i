...
QUEX_NAMESPACE_COMPONENTS_OPEN
     ...
     *          is equal to BufferEnd.                                        */
    {
        ...
        __quex_assert(base.buffer._content_character_index_begin == 0); 
        __quex_assert(base.buffer._memory._end_of_file_p != 0x0); 
        __quex_assert(ContentEnd > ContentBegin);
        QUEX_BUFFER_ASSERT_CONSISTENCY(&base.buffer);
        QUEX_BUFFER_ASSERT_NO_BUFFER_LIMIT_CODE(ContentBegin, ContentEnd);

        /* Move away unused passed buffer content. */
        QuexBuffer_move_away_passed_content(&base.buffer);

        /* Determine the insertion position. */
        QUEX_TYPE_CHARACTER*  insertion_p = base.buffer._memory._end_of_file_p;

        const size_t CopiedByteN = MemoryManager_insert((uint8_t*)insertion_p,  
                                                        (uint8_t*)(QuexBuffer_content_back(&base.buffer) + 1),
                                                        (uint8_t*)ContentBegin, 
                                                        (uint8_t*)ContentEnd);
        const size_t CopiedCharN = CopiedByteN / sizeof(QUEX_TYPE_CHARACTER);

        if( base.buffer._byte_order_reversion_active_f ) 
            __Buffer_reverse_byte_order(base.buffer._memory._end_of_file_p, insertion_p + CopiedCharN);

        /* When lexing directly on the buffer, the end of file pointer is always set. */
        QuexBuffer_end_of_file_set(&base.buffer, insertion_p + CopiedCharN);
     ...
     * where the statefulness of the converter cannot be controlled.              */
    {
        ...
        QuexBufferFiller_Converter<void>*  filler = (QuexBufferFiller_Converter<void>*)base.buffer.filler;
        __quex_assert(ContentEnd > ContentBegin);
        QUEX_BUFFER_ASSERT_CONSISTENCY(&base.buffer);
        ...
        filler->converter->convert(filler->converter, 
                                   ...
                                   &insertion_p,                 QuexBuffer_content_back(&base.buffer) + 1);

        if( base.buffer._byte_order_reversion_active_f ) 
            __Buffer_reverse_byte_order(base.buffer._memory._end_of_file_p, insertion_p);
        ...
        QUEX_BUFFER_ASSERT_NO_BUFFER_LIMIT_CODE(base.buffer._memory._end_of_file_p, insertion_p);

        QuexBuffer_end_of_file_set(&base.buffer, insertion_p);
    ...
    { 
        return QuexBuffer_text_end(&base.buffer); 
    }

    QUEX_INLINE QUEX_TYPE_CHARACTER*  
    QUEX_MEMFUNC(ANALYZER, buffer_fill_region_end)(__QUEX_SETTING_THIS_POINTER)
    { 
        return QuexBuffer_content_back(&base.buffer) + 1; 
    }
    ...
    {
        __quex_assert(base.buffer._memory._end_of_file_p + CharacterN <= base.buffer._memory._back);

        /* We assume that the content from '_end_of_file_p' to '_end_of_file_p + CharacterN'
         * has been filled with data.                                                        */
        if( base.buffer._byte_order_reversion_active_f ) 
            __Buffer_reverse_byte_order(base.buffer._memory._end_of_file_p, 
                                        base.buffer._memory._end_of_file_p + CharacterN);

        QUEX_BUFFER_ASSERT_NO_BUFFER_LIMIT_CODE(base.buffer._memory._end_of_file_p, 
                                                base.buffer._memory._end_of_file_p + CharacterN);

        /* When lexing directly on the buffer, the end of file pointer is always set.        */
        QuexBuffer_end_of_file_set(&base.buffer, 
                                   base.buffer._memory._end_of_file_p + CharacterN); 
    }
