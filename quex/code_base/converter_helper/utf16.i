...
__QUEX_CONVERTER_CHAR(utf16, utf8)(const uint16_t** input_pp, uint8_t** output_pp)
{
    ...
    if ( **input_pp <= (uint16_t)0x7f ) {
        ...
        *(*output_pp)++ = (uint8_t)(0x80 | (unicode & (uint16_t)0x3FFFF) >> 12);
        *(*output_pp)++ = (uint8_t)(0x80 | (unicode & (uint16_t)0xFFF)   >> 6);
        *(*output_pp)++ = (uint8_t)(0x80 | (unicode & (uint16_t)0x3F));
                                    ...
                                    uint16_t**        output_pp)
{
    if( **input_pp < (uint16_t)0xD800 || **input_pp > (uint16_t)0xE000 ) {
        ...
        *(*output_pp)++ = (uint16_t)(*(*input_pp)++ - (uint16_t)0xDC00);
    }
}
