...
internal structure the resulting state machine is more time and space efficient
than the original one.  

Figure \ref{fig:two-state-machines} shows a state machine consisting of four
states.  The dotted lines indicate 'exit', i.e. a path that is taken to exit
the state machine.  In the original state machine, as shown in figure
\ref{fig:two-state-machines}.a, the content of $x$ is incremented upon each
...
of no concern. The focus will be the operations that happen as a consequence of
state transitions.  Let $V_c$ name the specification of the set of variables
which are subject to such operations. $V_c$ may be any type of formal
description such as 'all variables related to input position storage' where the
number of variables remains arbitrary.  Let $op(i)$ name the operations applied
to the variables $V_c$ upon entry into the state machine, independent from
where the state is entered. Let $op_E(i,k)$ name operations applied upon entry,
...
That is, upon entry into a state $i$ the same operations $op(i)$ are on $V_c$
independently from which state it is entered.  A single-entry state is shown in
figure \ref{fig:se-vs-me}.a. The optimization transforms the single-entry state
machine into a \textit{multi-entry state machine}.  That is, the operations
$op_E(i,k)$ dependent on the transition from which the state is entered. A
...
states which are reachable from state $i$.

Along the transitions of a state machine a variety of operations may occur
which do not necessarily relate to the same subject. Some operations determine
the last accepted pattern, the input position to be restored upon acceptance,
the line and column numbers, the checksum value of a lexeme, or the sum of
character grapheme widths of the lexeme's characters, and so on. The term
'investigated behavior' is defined to specify a focus of analysis.  Let the
setting of required variables be defined as
...
With $V(k)$ as the setting before entry into a state $i$ and $V(i)$ as the
...
\label{eq:operation}
                         V(i) = op(i)(V(k))
\end{equation}
The aforementioned definitions can now specify the exact frame of an
investigated behavior of the state machine. 
\begin{definition}
...
'snapshot' of a variable $v$-s content upon entry into a mouth state. 
...
Let $R^v(i,k)$ and $R^v(i)$ signify the components of $R(i,k)$ which produce
$v\,\in\,V(i)$.  
\end{definition}
...
that produce a setting of variables \textit{independent} of $V_c$.

With the concepts of this section, the overall goal can be formulated. The
analysis transforms the given single-entry state machine based on transition
operations $op(i)$ into a multi-entry state machine based on entry operations
$op_E(i,k)$ and exit operations derived from recipes. Observing only the entry
and exit, the changes to $V_c$ must comply to the nominal behavior. 
...
$\varepsilon$ denote the not existing element. That is, if it is added to a
set, it does not change the sets content.
\begin{definition} $S^v(X)$, $S(X)$ -- Snapshots.
    ...
    \begin{equation}
        ...
        S(X) = \{ S^v(X) \,\forall\,\mbox{$v$ where $A(v)$ is used in $X$}.
    \end{equation}
...
\begin{equation} \label{eq:snapshot-map-difference-2b}
   S(R(i)) \neq S(R(i)) \exists\,v\,\,\mbox{with}\,\,S^v(R(i)) \neq S^v(R(i)) 
\end{equation}
From the last to equations it follows the following statement.
%%
\begin{statement}
   If the snapshot maps of two recipes differ, the recipes are unequal. 
...
\begin{equation} \label{eq:interference-snap-shot-maps}
    S^v(R(i)) = \begin{dcases*}
                  \varepsilon & if $R^v(i)$ is constant. \\
...
\end{definition}
%%
Both types of states, linear and mouth states, may be springs.  A spring can be
the starting point for the development of recipes, because it can derive a
recipe without knowing the predecessor's recipe. 
...
undetermined.  With the aforementioned discussion, an important conclusion can
be drawn.  Determined mouth states performed interference, and therefore must
have all their entries determined.  None of their entries can directly
originate in a dead-lock mouth state.
...
computed value of $v$ and $R^v(i) = \{ v = A(v) \}$ with $S^v(R(i))=i$.
...
\begin{equation}
    F_E^v(i,k) = \begin{dcases*}
                 ...
                 \end{dcases*}
\end{equation}
The snapshot information develops as
\begin{equation} \label{eq:run-time-interference-snapshot}
    S^v(i) = \begin{dcases*}
...
$R^v_d$ is the prototype of a determined predecessor recipe and $R^v_u$ is the
...
$S(R(s))=\emptyset$. So, if there was a snapshot from a dead-lock state
it would pass the spring. It holds in general
%%
\begin{equation} \label{eq:before-horizon}
        S(R^v_d)\,\cap\,U\,=\,\emptyset
\end{equation}
%%
From equation \eqref{eq:run-time-interference-snapshot} it follows that no
...
\end{equation}
%%
Undetermined entry recipes develop from the outputs of run-time interference 
recipes. Thus, there snapshots can only root back to horizons or their successor
states, i.e.
%%
...
\end{equation}
%%
For recipes to develop through a mouth state, they need to be the same. Also
their snapshot maps must be the same. However, equations
...
\end{equation}
%%
This in turn, is only true if $R^v_u$ and $R^v_d$ do not rely on stored values
at all, i.e. they are constant. This result is very important. When $R^v_u$
becomes determined, through subsequent accumulation, it is not possible that it
produces a new component $R^v(i)$ through interference. For both cases,
$op^v(i)$ being constant and non-constant, the output recipe $R(i)$ by run-time
interference remains correct.  The output recipes can be used for propagation
of recipes until other dead-lock states receive determined entries.  The
horizon moves, the set of dead-lock shrinks smaller and a new cycle
begins. This continues until not dead-lock state is left. 

Eventually, all entry recipes will be determined. It is now possible that those
entries which are flagged with $F_E^v(i,k)=true$ implement the entry operation
...
output recipe $R^v(i)$ if it was constant in itself.  Now, that all entry
recipes are determined, homogeneity can be achieved if the concatenation of
$op^v(i)\circ R(k)$ for all $k\,\in\,Pred(i)$ are of the same constant.
