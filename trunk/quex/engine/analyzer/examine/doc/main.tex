...
Let an operation $op^v(i)$ that assigns a constant to $v$ be called a
\textit{constant operation}. It is something special, because it is independent
of $V(i)$ and the concatenation to determine a recipe $R(k)$ becomes
%%
\begin{equation}
    R(k)\,=\,op^v(k)\,\circ\,R(p)\,=\,op^v(k)\,\,\mbox{where $p\in Pred(k)$}
\end{equation}
%%
A state where all operations are constant, a recipe can be determined
\extit{independently} of other recipes. It is therefore suited as starting
point for the derivation of other recipes.
%%
\begin{definition} Initial Spring, Spring
...
\end{definition}
%%
Both, linear states and mouth states may be equally candidates for spring.
...
$x:=x+1$ for all states. Let $A_a(x)$ hold the value of $x$ upon entry the
