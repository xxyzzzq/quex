...
predecessor recipes ends up with the exact same formula for cautious
interference as in equation \ref{eq:cautious-output-recipe}. Let the
...
the entry operations store $v$ in $A(v)$ and $R^v(i)=\{ v \coloneqq A(v)\}$ with
$S^v(R^v(i))=i$.  Cautious interference can, therefore, be implemented by
accumulation of the undetermined recipe from equation
\eqref{eq:undetermined-recipe} with $op(i)$ followed by 'normal'
interference $\blacksquare$.
