...
#   define self (*this)
    ...
    QUEX_INLINE void   
    QUEX_MEMFUNC(ANALYZER, send)(const QUEX_TYPE_TOKEN_WITH_NAMESPACE_ID ID) 
    {
    ...
    QUEX_INLINE void   
    QUEX_MEMFUNC(ANALYZER, send_n)(const int RepetitionN, QUEX_TYPE_TOKEN_WITH_NAMESPACE_ID ID) 
    {
...
#   ifndef __QUEX_SETTING_PLAIN_C
    ...
    QUEX_TYPE_ANALYZER::send(const QUEX_TYPE_TOKEN_WITH_NAMESPACE_ID ID, X0_T X0) 
    {
        self_send1(ID, X0);
    }

    template <typename X0_T, typename X1_T> QUEX_INLINE void   
    QUEX_TYPE_ANALYZER::send(const QUEX_TYPE_TOKEN_WITH_NAMESPACE_ID ID, X0_T X0, X1_T X1) 
    {
        self_send2(ID, X0, X1);
    }

    template <typename X0_T, typename X1_T, typename X2_T> QUEX_INLINE void   
    QUEX_TYPE_ANALYZER::send(const QUEX_TYPE_TOKEN_WITH_NAMESPACE_ID ID, X0_T X0, X1_T X1, X2_T X2) 
    {
        self_send3(ID, X0, X1, X2);
    }

    template <typename X0_T, typename X1_T, typename X2_T, typename X3_T> QUEX_INLINE void   
    QUEX_TYPE_ANALYZER::send(const QUEX_TYPE_TOKEN_WITH_NAMESPACE_ID ID, X0_T X0, X1_T X1, X2_T X2, X3_T X3) 
    {
