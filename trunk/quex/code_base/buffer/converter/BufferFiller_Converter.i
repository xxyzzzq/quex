 ...
 * data even if they report that 'iterator==end'. The final decision is left to
 * the converter in its first call to 'convert()'.  The assumption 'iterator ==
 * end => reload required' does not hold here for the general case.                                                              
 *                   
...
#       endif
    ...
    while( 1 + 1 == 2 ) {
        ...
        /* The raw buffer filler requires the iterator's character index to be
         * up-to-date.                                                       */
    ...
    if( ConvertedCharN != (ptrdiff_t)N ) {
        /* The buffer was not filled completely, because the end of the file
         * was reached.                                                      */
...
{ 
    ...
    /* The raw buffer iterator stands on the next character to be read. In
     * general it holds that the raw_buffer's iterator points to the first byte
     * of the next character to be converted when the next user buffer is to be
     * filled.                                                               */
...
/* BufferFiller's seek sets the input position for the next character read
 * in the stream.  This is different from the Buffer's seek that sets the
 * _read_p at a specific character position. Also, the ByteLoader's seek
 * sets the stream to a stream position, not a character position.           */
{ 
    ...
    /* Seek_character_index(Pos) means that the next time when a character
     * buffer is to be filled, this has to happen from position
     * 'CharacterIndex'. 
     *
     * The reference for character positioning is **not** directly the stream.
     * Moreover, it is the internal raw_buffer.position. It determines what
     * characters are converted next into the user's buffer.                 */

    if( Index == buffer->iterators_character_index ) { 
        return;                                        /* Nothing to be done */
    }

    /* Some type of converters (actually, at the time of this writing, only
     * IBM's ICU) require a reset as soon as the conversion is discontinued,
     * i.e. the stream of character's is disrupted. The reset happens to cope
     * with some internal 'statefulness'.                                    */
    if( me->converter->on_conversion_discontinuity ) {
        me->converter->on_conversion_discontinuity(me->converter);
    }

    /* Depending on the character encoding, the seek procedure can be
     * optimized. There are the following two cases:
     *
     * (1) The coding uses **fixed** character widths (such as ASCII, UCS2,
     *     UCS4, etc.) where each character always occupies the same amount of
     *     bytes. Here, offsets can be **computed**. This makes things faster.
     *
     * (2) The coding uses **dynamic** character width (e.g. UTF-8). Here the
     *     position cannot be computed. Instead, the conversion must start from 
     *     a given 'known' position until one reaches the specified character 
     *     index.                                                            */
    if( ! me->converter->dynamic_character_size_f ) { 
         ...
         *
         * Setting the iterator to the begin of the raw_buffer initiates a
         * conversion start from this point.                                 */

        /* hint index must be valid (i.e. != -1)                             */
        if( Index == Hint_Index && Hint_Index != (ptrdiff_t)-1 ) { 

            /* The 'read_characters()' function works on the content of the
