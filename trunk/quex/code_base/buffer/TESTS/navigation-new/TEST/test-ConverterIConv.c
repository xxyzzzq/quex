...
static void test_file(QUEX_NAME(Converter)* converter, bool LinearF, const char* FileName, const char* FileStem);
...
test(E_ConverterTestType CTT, bool LinearF, size_t BPC)
{
    ...
    switch( BPC ) {
    case 4:  test_file(converter, LinearF, file_4, "examples/languages");   /* only with UCS4         */
    case 2:  test_file(converter, LinearF, file_2, "examples/small");       /* only with UCS4, UCS2   */
    case 1:  test_file(converter, LinearF, file_1, "examples/festgemauert");/* with UCS4, UCS2, ASCII */
             break;
...
static void
test_file(QUEX_NAME(Converter)* converter, bool LinearF, const char* FileName, const char* FileStem)
{
    ...
    printf("# byte_loader:binary_mode_f: %i;\n", (int)filler->byte_n_per_character);
    printf("# filler:byte_n_per_character: %i;\n", (int)filler->byte_n_per_character);
    if( LinearF ) {
