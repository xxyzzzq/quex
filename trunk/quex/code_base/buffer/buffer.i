...
#   define CLASS      buffer<CharacterCarrierType>   
                                  ...
                                  size_t FallBackN) 
    {
        ...
        //
        if( buffer_memory == 0x0 ) _buffer.front = new character_type[BufferSize];      
        else                       _buffer.back  = buffer_memory;
        _buffer.back = _buffer.front + BufferSize - 1;
        ...
        *(_buffer.front) = CLASS::BLC; // set buffer limit code
        *(_buffer.back)  = CLASS::BLC; // set buffer limit code
...
#       endif
        ...
        on_overflow = default_buffer_on_overflow_handler;
    ...
    TEMPLATE inline
        CLASS::~buffer() {
            delete [] _buffer; // If buffer has been allocated from outside, then no deletion
        }
